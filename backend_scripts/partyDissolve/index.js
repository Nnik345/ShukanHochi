...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (party.leaderId !== sub) {
      return corsJson(403, { error: 'Only the party leader can dissolve the party' });
    }

    await parties.deleteOne({ partyId });
    await db.collection(PARTY_INVITES).deleteMany({ partyId });
